...
module.exports = async function (helper) {
  ...
  try{
    let info = await projectInfo(parentFolder,'CountUpUserInput');

    info.contents = stripSpaces(info.contents);


    if(!info.contents.includes('Console.ReadLine()')){
      return helper.fail('You must take the starting value from the user via the ReadLine method');
    }
    ...
    let runResults = await test_inputs(15, command, [START]);
